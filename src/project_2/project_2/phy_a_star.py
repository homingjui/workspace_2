...
class slam_navigation_node(Node):
    def __init__(self):
        ...
        self.rout = np.array([[ 0.2  , 1.0  ],
                              [ 0.2  , 3.271],
                              [-0.684, 3.947],
                              [-0.684, 6.791],
                              [-1.25 , 7.01 ],
                              [-1.25 , 8.85 ]],dtype=np.float)
    ...
    def get_scan(self,data):
        ...
        for index,item in enumerate(data.ranges):
            if index  > 390 and index < 530:
                item = 100
            #    continue
            now_ang = data.angle_min+data.angle_increment*index

            temp_ang = now_ang+self.car_tf-np.pi/2
    ...
    def controller(self):
        ...
        print((self.now_dot,go_dot,self.add_path_n,deg,dist,deg_path))
        if abs(deg_path)>60 or dist<0.45:
            if not self.add_path[0][0]==None:
        ...
        path_imgt = np.array(a_img)
        a_img2 = np.array(a_img)
        img = np.zeros(( expend_y_max-expend_y_min+expand*2,
                         expend_x_max-expend_x_min+expand*2,3))
        
        cv2.circle(path_imgt, drew_car, int(car_w*1.25), 1, -1)
        cv2.line(path_img, drew_car, rout_drew[go_dot+1],
                                        1,path_w)
        path_img = cv2.bitwise_and(path_img, path_imgt)
        
        for i in drew_scan:
            if i[2]>0:
                cv2.circle(a_img, i[:2], car_w, 1, -1)
        #cv2.circle(a_img, drew_car, car_w*2, 0, -1)
        cv2.line(path_img, rout_drew[go_dot], rout_drew[go_dot+1],
                                        1,path_w)
       
        #cv2.imshow('img2',cv2.bitwise_or(path_img, a_img))
        #cv2.waitKey(1)
        ...
        if 1 in add_img:
            ...
            for i in drew_scan:
                if i[2]>0:
                    cv2.circle(a_img2, i[:2], int(car_w*1.2), 1, -1)
                else:
                    cv2.circle(a_img2, i[:2], car_w, 2, -1)
    
            cv2.circle(a_img2, drew_car, car_w, 0, -1)

            #cv2.imshow('img',a_img2)
            #cv2.waitKey(0)
            
            next_p = rout_drew[self.now_dot+1]
            if a_img2[next_p[1],next_p[0]] > 0:
        ...
        cv2.line(img, drew_car, way, (0.5,0.5,0), int(car_w/2))


        for i in drew_scan:  ## two type point
            if i[2]>0:
                cv2.circle(img, i[:2], car_w, (0.2,0.2,0.2), -1)
